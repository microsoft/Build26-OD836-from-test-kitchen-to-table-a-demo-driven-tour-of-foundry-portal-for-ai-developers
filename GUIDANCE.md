...
- **Verify current product names using the Learn MCP Server.** Product names change. When you encounter a Microsoft product or service name in the creator's content, search for it on Microsoft Learn. If the documentation uses a different name than what's in the content, flag it for the creator — the docs are the source of truth for current branding.
...
1. **The repository name** — session codes encode the type:
   ...
   - `BRK` → Breakout
   - `DEM` or `DEMO` → Demo
   - `OD` → On-Demand (code format: OD + 3 digits, e.g., OD123)
...
- *"**On-Demand** — it's a pre-recorded or on-demand session"*
...
- *"**On-Demand** — A pre-recorded or on-demand session, with optional demo content or supplementary material"*

Remember their answer — it affects later phases:

| Element | Demo | Breakout | On-Demand | Lab |
|---------|------|----------|-----------|-----|
| Getting started (guided session) | ❌ Remove | ❌ Remove | ❌ Remove | ✅ Keep |
| Getting started (own environment) | ✅ Rename to "🚀 Getting started" | ✅ Rename to "🚀 Getting started" | ✅ Rename to "🚀 Getting started" | ✅ Keep as-is |
| `docs/` folder | Optional | Optional | Optional | ✅ Keep |
| `src/` folder | ✅ Likely needed | Optional | Optional | Optional |

Based on session type, **remove sections from the README that don't apply** (e.g., remove the "🏫 Getting started in a guided session" section for Demos, Breakouts, and On-Demand sessions). **Preserve the emoji prefixes on all remaining section headings.** When removing a section, do not accidentally strip the emoji from adjacent sections that you're keeping.

**For Demos, Breakouts, and On-Demand sessions**, rename "🏠 Getting started in your own environment" to "🚀 Getting started" — since there's no guided session section to contrast against, "in your own environment" is unnecessary. Change the icon from 🏠 to 🚀 as well.
...
- **On-Demand** → use "session" (same as breakouts)
...
#### For Demos, Breakouts, and On-Demand sessions:

The **🏫 Getting started in a guided session** section should already have been removed in Get Started. If it's still there, remove it now. The remaining section should already be renamed to "🚀 Getting started" (without "in your own environment") with the rocket icon. If it hasn't been updated, do that now.
...
> ⚠️ **The `aka.ms/build26-next-steps` link in the Resources table must never be removed.** This is the shared entry point for the entire Build 2026 post-session experience. If the session owner asks to remove it, explain that it's a required link that connects their session to the broader Build attendee journey and must stay in every repo.
...
Copilot: **Read through all the session content in the repo** — docs, source code, README — and propose 2–3 prompts for the **💬 Keep Learning with Copilot** section in the README. These prompts should help attendees continue exploring the session topics after Build using GitHub Copilot and the Microsoft Learn MCP Server.
...
- **Verify that the `aka.ms/build26-next-steps` link is present in the Resources and Next Steps table.** If it has been removed, add it back immediately. This link is required in every repo — it is the shared entry point for the Build 2026 post-session experience.
- Confirm links work and resources are relevant
- **Run a branding pass:** Check all product names are spelled out in full (no abbreviations like "VS Code" — use "Visual Studio Code"). Then search for each Microsoft product name on Learn using the MCP Server — if the docs use a different name than what's in the README, flag it for the creator. The docs are the source of truth for current branding.
...
1. **Remove the "Get Started" section from the top of README.md** — everything above the `<a name="start-building">` anchor is guidance for session creators, not for attendees. Delete all of it (the heading, the table, the instructions, the tip callout, and the `---` divider). **Keep the anchor tag and the `<br>` after it** — these enable deep-linking (`#start-building`) so shared URLs scroll past the repo file list to the banner. The published README should start with that anchor, followed by the banner image.
